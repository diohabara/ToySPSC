...
| ネットワーク | パケット受信スレッド → 処理スレッドへのメッセージパッシング |
...
[[nodiscard]] auto try_pop() noexcept -> std::optional<T> {
  ...
  auto value = buffer_[head & kMask];                        // ③ データ読み出し
...
- `requires std::is_trivially_copyable_v<T>`: T が trivially copyable（`memcpy` で安全にコピーできる）ことを要求。`int32_t`, `double`, POD 構造体などが該当。`std::string` や `std::vector` は不可
